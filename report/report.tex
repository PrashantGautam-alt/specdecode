...
\usepackage{graphicx}
\graphicspath{{figures/}{report/figures/}}  % find figures whether compiling from report/ or repo root
\usepackage{booktabs}
\usepackage{hyperref}
\usepackage{xcolor}
\usepackage{listings}
...
\begin{titlepage}
    ...
    \begin{abstract}
        ...
        measured offline and a fixed node budget is filled by greedy selection over
        each candidate's expected acceptance. The selection \emph{automatically} drops
        the weakest head (assigning it zero nodes -- an emergent version of the
...
\begin{figure}[H]
\centering
\includegraphics[width=0.8\textwidth]{k_sweep.png}
\caption{Speedup peaks at $K{=}4$ even though acceptance keeps rising with $K$: the certain,
linear draft cost eventually overtakes the diminishing, conditional acceptance gain.}
\end{figure}
...
This is a speed--quality tradeoff: raise $T$ for more speed until the text stops being coherent,
then back off -- the best operating point is the highest $T$ that still reads cleanly. Two
...
nodes are selected. Each step spends a slot on the candidate most likely to extend an
already-accepted path, so the budget goes where it raises expected accepted tokens the most.
Because a child becomes a candidate only after its parent is chosen, parents precede children,
exactly the ordering the decoder requires; the topology is computed once and stored.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.7\textwidth]{calibrated_vs_cartesian.png}
\caption{At the same 14-node budget, the calibrated tree accepts more tokens per round than the
uniform Cartesian tree, by spending the budget on the most-likely-accepted candidates.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{results_ladder.png}
\caption{Speedup by configuration. Blue: lossless. Green: the calibrated tree (the
contribution), also lossless. Orange: typical acceptance (lossy). Red: slower than baseline.}
\end{figure}
...
\textbf{A greedy criterion can re-derive a hand-found result.} The optimal-$K$ tradeoff
was first observed by hand: a sixth head made greedy slower. The calibrated tree's
greedy node-selection then produced the same conclusion automatically, assigning zero
nodes to the weakest head -- its candidates had the lowest expected acceptance, so they
were never worth a slot. Selecting nodes by expected accepted tokens made the optimal-$K$
effect fall out of the tree construction itself, instead of being something I tuned by
hand -- the most satisfying moment of the project.
...
\begin{itemize}
    \item \textbf{Adaptive draft length.} Treat the head count / tree depth as a control
          ...
          on predictable text and shrinking it on hard text. This is the same idea the
          calibrated tree applies \emph{offline}, made \emph{online} -- adapting the draft to
